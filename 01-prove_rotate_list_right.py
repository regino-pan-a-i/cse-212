...
def rotate_list_right(data, amount):
	...
	# Slicing the list with the amount needed from back to front.
	moving_values = data[len(data)- amount: ]
	
	# Only leaving in the list the elements that will not move.
	data = [value for value in data if value not in moving_values]
	
	# Adding the moving elements to the begining of the list.
	results = moving_values + data
